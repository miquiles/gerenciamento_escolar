...
class PHPFrodo
{
    ...
    public function __construct()
    {
        ...
        $this->assign( 'timeNow', date( 'dmyhis' ) );
    ...
    public function database( $dbname = null )
    {
        try
        {
            ...
            if ( $this->adapter == null )
            {
                ...
                {
                    //conexao PDO
                    if ( !is_callable( array( 'PDO', '__construct' ) ) )
                    {
                        throw new Exception( "database: Você não tem o PDO habilitado no php.ini, habilite-o ou utilize o método adapter()." );
                    }
                    $this->objBanco = new Conexao( $databases[$this->dbname] );
                }
            }
            else
            {
    ...
    public function urlmodr( $key, $nkey=null, $reverse=null, $sep=null )
    {
        $group_a = array( 'À', 'Á', 'Â', 'Ã', 'Ä', 'Å', 'Æ', 'Ç',
            'È', 'É', 'Ê', 'Ë', 'Ì', 'Í', 'Î', 'Ï', 'Ð',
            'Ñ', 'Ò', 'Ó', 'Ô', 'Õ', 'Ö', 'Ø', 'Ù', 'Ú',
    ...
    public function search( $haystack, $needle, $index = null )
    {
        ...
        while ( $it->valid() )
        {
            if ( ((isset( $index ) AND ($it->key() == $index)) OR (!isset( $index ))) AND (strpos( $it->current(), $needle ) !== false) )
            {
                ////$it->current() == $needle 
                $resultkeys[] = $aIt->key();
                ////return $aIt->key(); 
    ...
    public function postIndexDrop( $index )
    {
        ...
        if ( $remove || isset( $this->post_fields["$remove"] ) )
        {
